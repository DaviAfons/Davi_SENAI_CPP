...
void adicionarTarefa(std::queue<std::string> &lista, std::string missao)
{
    lista.push(missao); // Adiciona a nova tarefa à fila
}
...
void marcarConcluida(std::queue<std::string> &lista)
{
    ...
    {                                                                    // Verifica se a lista não está vazia
        std::cout << "Missao concluida: " << lista.front() << std::endl; // Exibe a tarefa a ser marcada como concluída
        lista.pop();                                                     // Remove a tarefa concluída da fila
    }
    else
    {
        std::cout << "Missao nao concluida!\n"; // Exibe mensagem se a lista estiver vazia
    }
}
...
int main()
{
    ...
    adicionarTarefa(listaAfazeres, "Salvar um gato de uma arvore");
